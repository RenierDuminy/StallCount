...
class ScorekeeperApp {
  ...
  /**
   * Tear down listeners and timers for SPA environments
   */
  destroy() {
    try {
      ...
      if (this.persistenceManager && typeof this.persistenceManager.stopAutoSave === 'function') {
      ...
      if (this.loadingManager && typeof this.loadingManager.stop === 'function') {
        this.loadingManager.stop();
      }
    } catch (error) {
      console.warn('ScorekeeperApp destroy encountered an issue:', error);
    }
  }
}
...
export async function bootstrapScorekeeper(force = false) {
  if (initialized && !force && app) {
    return app;
  }
...
export function destroyScorekeeper() {
  if (app && typeof app.destroy === 'function') {
    app.destroy();
  }
  app = null;
  initialized = false;
  delete window.ScorekeeperApp;
}
