...
const buildDivisionStandings = (division, matchesByPool = {}) => {
  ...
  (division?.pools || []).forEach((pool) => {
    ...
    finishedMatches.forEach((match) => {
      ...
      const teamA = teamAId ? standings.get(teamAId) : null;
      const teamB = teamBId ? standings.get(teamBId) : null;
            ...
            {eventData.divisions.length === 0 ? (
              ...
              <>
                {eventData.divisions.map((division) => {
                  const divisionStandings = buildDivisionStandings(
                    division,
                    matchesByPool,
                  );
                    ...
                    <Card key={division.id} className="space-y-4 border border-white/80 p-6">
                      ...
                      <Panel variant="muted" className="space-y-2 p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <Chip>{divisionStandings.length}</Chip>
                        </div>
                        <TeamTable
