      ...
      <table className="min-w-full text-sm">
        ...
        <tbody>
          {rows.map((row, index) => (
            ...
            >
              <td className="px-3 py-2">{row.seed !== null ? `#${row.seed}` : "--"}</td>
              <td className="px-3 py-2">
                {row.shortName ? `${row.name} (${row.shortName})` : row.name}
              </td>
              <td className="px-3 py-2">{row.pool}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
export default function Event5779WorkspacePage() {
  ...
  const divisionRows = useMemo(() => {
    ...
    return eventData.divisions.map((division) => ({
      ...
      teams: buildDivisionTeams(division),
    }));
  }, [eventData]);
      ...
      <SectionShell as="main" className="space-y-6 py-8">
        ...
        <Card className="space-y-4 p-5 sm:p-6">
          <SectionHeader
            ...
            description="View each division roster grouped from configured pools."
          />
          {loading && divisionRows.length === 0 ? (
